...
module Sources
  class Amazon < Base
    def initialize
      @@aws_signer = ::AwsProductSign.new(:access_key => "0W69P3FBHTG0W578E202", :secret_key => "w0JfEAjfSAGO95DTlaTSGxn2MF8zYA+iyWHXzlgM")
      @@amazon = Net::HTTP.start("webservices.amazon.com", 80)
    end
    ...
    def search(title)
      ...
      request = "/onca/xml?#{query_string}"
      response = @@amazon.get(request, headers)
    ...
    def parse(response)
      ...
      (doc/:Item).collect do |item|
        { :title => (item/:Title).inner_text, :id => (item/:ASIN).inner_text, :url => "http://www.amazon.com/gp/product/#{(item/:ASIN).inner_text}"}
      end
    end
  end
end
